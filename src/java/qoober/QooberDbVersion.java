...
import qoober.db.DbVersion;

class QooberDbVersion extends DbVersion {
        ...
        switch (nextUpdate) {
            ...
            case 40:
                apply("CREATE TABLE IF NOT EXISTS account (db_id IDENTITY, id BIGINT NOT NULL, "
                        ...
                        + "forged_balance BIGINT NOT NULL, paramining_balance BIGINT NOT NULL, active_lessee_id BIGINT, parent BIGINT, "
                        + "last_paramining_start INT NOT NULL, height INT NOT NULL, latest BOOLEAN NOT NULL DEFAULT TRUE)");
            case 41:
            ...
            case 199:
                qoober.db.FullTextTrigger.init();
                apply(null);
            case 200:
            ...
            case 250:
                apply("CREATE ALIAS CAN_BE_TRIMMED FOR \"qoober.db.TrimmableDbTable.canBeTrimmed\"");
            case 251:
                apply("CREATE TABLE IF NOT EXISTS account_struct (db_id IDENTITY, id BIGINT NOT NULL, balance BIGINT NOT NULL, " +
                        "height INT NOT NULL, latest BOOLEAN NOT NULL DEFAULT TRUE)");
            case 252:
                apply("CREATE UNIQUE INDEX IF NOT EXISTS account_struct_id_height_idx ON account_struct (id, height DESC)");
            case 253:
                apply("CREATE TABLE IF NOT EXISTS account_parent_links (db_id IDENTITY, id BIGINT NOT NULL, " +
                        "parent BIGINT NOT NULL, distance INT NOT NULL, height INT NOT NULL)");
            case 254:
                apply("CREATE INDEX IF NOT EXISTS account_parent_links_id_idx ON account_parent_links (id)");
            case 255:
                apply("CREATE INDEX IF NOT EXISTS account_parent_links_height_idx ON account_parent_links (height DESC)");
            case 256:
                apply("CREATE TABLE IF NOT EXISTS account_paramining (db_id IDENTITY, id BIGINT NOT NULL, balance BIGINT NOT NULL, " +
                        "struct_balance BIGINT NOT NULL, height INT NOT NULL)");
            case 257:
                apply("CREATE UNIQUE INDEX IF NOT EXISTS account_paramining_id_height_idx ON account_paramining (id, height DESC)");
            case 258:
                apply("CREATE TABLE IF NOT EXISTS paratax (db_id IDENTITY, all_balance BIGINT NOT NULL, height INT NOT NULL)");
            case 259:
                return;
            default:
