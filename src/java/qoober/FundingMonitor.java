...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = Qoober.getIntProperty("qoober.maxNumberOfMonitors");
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Qoober.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Qoober.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
    ...
    private static void processNxtEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                Qoober.getTransactionProcessor().broadcast(transaction);
                monitoredAccount.height = Qoober.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("%s funding transaction %s for %f %s submitted from %s to %s",
                        Constants.COIN_SYMBOL, transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_QBR,
