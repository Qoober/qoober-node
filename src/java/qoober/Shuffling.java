...
import qoober.crypto.AnonymouslyEncryptedData;
import qoober.crypto.Crypto;
import qoober.db.DbClause;
import qoober.db.DbIterator;
import qoober.db.DbKey;
import qoober.db.DbUtils;
import qoober.db.VersionedEntityDbTable;
...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Qoober.getBooleanProperty("qoober.deleteFinishedShufflings");
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (holdingType != HoldingType.NXT) {
                    participantAccount.addToBalanceNQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_QNT);
                }
            }
        }
        for (byte[] recipientPublicKey : recipientPublicKeys) {
            ...
            if (holdingType != HoldingType.NXT) {
                recipientAccount.addToBalanceAndUnconfirmedBalanceNQT(event, this.id, Constants.SHUFFLING_DEPOSIT_QNT);
            }
        }
    ...
    private void cancel(Block block) {
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (participantAccount.getId() != blamedAccountId) {
                    if (holdingType != HoldingType.NXT) {
                        participantAccount.addToUnconfirmedBalanceNQT(event, this.id, Constants.SHUFFLING_DEPOSIT_QNT);
                    }
                } else {
                    if (holdingType == HoldingType.NXT) {
                        participantAccount.addToUnconfirmedBalanceNQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_QNT);
        ...
        if (blamedAccountId != 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                Logger.logDebugMessage("Shuffling penalty %f %s awarded to forger at height %d", ((double)fee) / Constants.ONE_QBR, Constants.COIN_SYMBOL, block.getHeight() - i - 1);
            }
            ...
            Logger.logDebugMessage("Shuffling penalty %f %s awarded to forger at height %d", ((double)fee) / Constants.ONE_QBR, Constants.COIN_SYMBOL, block.getHeight());
        }
