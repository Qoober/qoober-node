...
import qoober.AccountLedger.LedgerEntry;
import qoober.AccountLedger.LedgerEvent;
import qoober.AccountLedger.LedgerHolding;
import qoober.crypto.Crypto;
import qoober.crypto.EncryptedData;
import qoober.db.*;
import qoober.util.Convert;
import qoober.util.Listener;
import qoober.util.Listeners;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, Qoober.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Qoober.getBlockchain().getHeight();
        }
    ...
    private static final VersionedEntityDbTable<Account> accountTable = new VersionedEntityDbTable<Account>("account", accountDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            ...
            if (height > Qoober.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Qoober.getBlockchain().getHeight());
            }
        }
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        @Override
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Qoober.getBlockchainProcessor().getMinRollbackHeight()) {
    ...
    // Paramining tables
    private static final TrimmableDbTable<Account> accountStructTable = new TrimmableDbTable<Account>("account_struct", accountDbKeyFactory, true) {
        @Override
        public void trim(int height) {
            super.trim(height - Constants.GUARANTEED_BALANCE_CONFIRMATIONS);
        }
    };

    private static final TrimmableDbTable<Account> accountParaminingTable = new TrimmableDbTable<Account>("account_paramining", accountDbKeyFactory, false) {
        @Override
        public void trim(int height) {
            try (Connection con = db.getConnection();
                 PreparedStatement pstmtSelect = con.prepareStatement("SELECT A1.id, A1.last_paramining_start FROM account AS A1"
                         + " WHERE A1.height = (SELECT MAX(A2.height) FROM account AS A2 WHERE A2.id = A1.id AND A2.height < ?)");
                 PreparedStatement pstmtDelete = con.prepareStatement("DELETE FROM account_paramining WHERE id = ?"
                         + " AND height < ? LIMIT " + Constants.BATCH_COMMIT_SIZE)) {
                ...
                try (ResultSet rs = pstmtSelect.executeQuery()) {
                    ...
                    while (rs.next()) {
                        ...
                        pstmtDelete.setInt(2, rs.getInt("last_paramining_start"));
                        do {
    ...
    private static final DerivedDbTable accountParentLinksTable = new DerivedDbTable("account_parent_links");
    private static final DerivedDbTable parataxTable = new DerivedDbTable("paratax");

    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Qoober.getBooleanProperty("qoober.enablePublicKeyCache") ?
    ...
    static {
        ...
        Qoober.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            Qoober.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    private Account(ResultSet rs, DbKey dbKey) throws SQLException {
        ...
        this.paraminingBalanceNQT = rs.getLong("paramining_balance");
        this.activeLesseeId = rs.getLong("active_lessee_id");
        this.parent = rs.getLong("parent");
        this.lastParaminingStart = rs.getInt("last_paramining_start");
    ...
    private void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account (id, "
                + "balance, unconfirmed_balance, forged_balance, paramining_balance, parent, last_paramining_start, "
            ...
            pstmt.setLong(++i, this.paraminingBalanceNQT);
            pstmt.setLong(++i, this.parent);
            pstmt.setInt(++i, this.lastParaminingStart);
    ...
    private void save() {
        if (balanceNQT == 0 && unconfirmedBalanceNQT == 0 && forgedBalanceNQT == 0 && paraminingBalanceNQT == 0 &&
                activeLesseeId == 0 && parent == 0 && controls.isEmpty()) {
    ...
    public long getParaminingBalanceNQT() {
        return paraminingBalanceNQT;
    }

    public long getEffectiveBalanceNXT() {
        return getEffectiveBalanceNXT(Qoober.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceNXT(int height) {
        if (height <= Constants.GUARANTEED_BALANCE_CONFIRMATIONS) {
            Account genesisAccount = getAccount(id, 0);
            return genesisAccount == null ? 0 : genesisAccount.getBalanceNQT() / Constants.ONE_QBR;
        }
        ...
        try {
	        ...
	        return effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_QNT ? 0 : effectiveBalanceNQT / Constants.ONE_QBR;
        } finally {
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Qoober.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Qoober.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Qoober.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = Qoober.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Qoober.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Qoober.getBlockchain().getHeight() - 1) {
    ...
    void addToBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        long oldBalance = this.balanceNQT;
        this.balanceNQT = Math.addExact(this.balanceNQT, totalAmountNQT);
        calculateBalances(totalAmountNQT, oldBalance, eventId);
    ...
    private void calculateBalances(long amountNQT, long oldBalance, long eventId) {
        long paramining = 0;

        if (amountNQT < 0) {
            paramining = Paramining.calculate(this.id);
            if (paramining > 0) {
                this.balanceNQT = Math.addExact(this.balanceNQT, paramining);
                this.unconfirmedBalanceNQT = Math.addExact(this.unconfirmedBalanceNQT, paramining);
                this.paraminingBalanceNQT = Math.addExact(this.paraminingBalanceNQT, paramining);

                if (AccountLedger.mustLogEntry(this.id, true))
                    AccountLedger.logEntry(new LedgerEntry(LedgerEvent.PARAMINING, eventId, this.id,
                            LedgerHolding.UNCONFIRMED_NXT_BALANCE, null, paramining, this.unconfirmedBalanceNQT));
                if (AccountLedger.mustLogEntry(this.id, false))
                    AccountLedger.logEntry(new LedgerEntry(LedgerEvent.PARAMINING, eventId, this.id,
                            LedgerHolding.NXT_BALANCE, null, paramining, this.balanceNQT));

                Account.getAccount(Genesis.CREATOR_ID).addToBalanceAndUnconfirmedBalanceNQT(null, 0, -paramining);
            }
        }

        if (amountNQT < 0 || (oldBalance < Constants.MIN_PARAMINING_BALANCE_QNT &&
            balanceNQT >= Constants.MIN_PARAMINING_BALANCE_QNT && balanceNQT < Constants.MAX_PARAMINING_BALANCE_QNT))
            lastParaminingStart = Qoober.getBlockchain().getHeight();

        amountNQT = Math.addExact(amountNQT, paramining);

        addToGuaranteedBalanceNQT(amountNQT);
        addToStructBalanceNQT(amountNQT);
    }
    ...
    private void addToStructBalanceNQT(long amountNQT) {
        if (amountNQT == 0)
            return;

        int blockchainHeight = Qoober.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_struct (id, balance, height, latest) " +
             "KEY (id, height) SELECT id, balance + ?, ?, TRUE FROM account_struct WHERE latest = TRUE AND id IN " +
             "(SELECT parent FROM account_parent_links WHERE id = ?)"))
        {
            pstmt.setLong(1, amountNQT);
        ...
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("UPDATE account_struct SET latest = FALSE WHERE latest = TRUE " +
                     "AND height < ? AND id IN (SELECT parent FROM account_parent_links WHERE id = ?)"))
        {
    ...
    public void initParaminingTables() {
        int blockchainHeight = Qoober.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement(
                     "INSERT INTO account_struct (id, balance, height, latest) VALUES (?, 0, ?, TRUE)"))
        {
            pstmt.setLong(1, this.id);
            pstmt.setInt(2, blockchainHeight);
            pstmt.executeUpdate();
    ...
    public void setGenesisParent() {
        if (parent == 0)
            parent = Genesis.CREATOR_ID;
    }

    public void setParentIfFirst(long parentId, long amountNQT) {
        if (parent == 0 && amountNQT > 0) {
            parent = parentId;
            ...
            try (Connection con = Db.db.getConnection();
                 PreparedStatement pstmt = con.prepareStatement(
                         "INSERT INTO account_parent_links (id, parent, distance, height) VALUES (?, ?, 1, ?)"))
            {
                pstmt.setLong(1, this.id);
                pstmt.setLong(2, parentId);
                pstmt.setInt(3, blockchainHeight);
                pstmt.executeUpdate();
            } catch (SQLException e) {
                throw new RuntimeException(e.toString(), e);
            }
            ...
            try (Connection con = Db.db.getConnection();
                 PreparedStatement pstmt = con.prepareStatement("INSERT INTO account_parent_links (id, parent, " +
                 "distance, height) SELECT ?, parent, distance + 1, ? FROM account_parent_links WHERE id = ? AND distance < ?"))
            {
                pstmt.setLong(1, this.id);
                pstmt.setInt(2, blockchainHeight);
                pstmt.setLong(3, parentId);
                pstmt.setInt(4, Constants.MAX_ACCOUNT_STRUCT_SIZE);
                pstmt.executeUpdate();
            } catch (SQLException e) {
                throw new RuntimeException(e.toString(), e);
            }

            initParaminingTables();

            Logger.logDebugMessage("Account %s has been linked to parent account %s",
                    Long.toUnsignedString(getId()), Long.toUnsignedString(getParent()));
        }
    }
    ...
    public int getLastParaminingStart() {
				if(lastParaminingStart == 0 && balanceNQT < Constants.MIN_PARAMINING_BALANCE_QNT){
					return Qoober.getBlockchain().getHeight();
				}
        return lastParaminingStart;
    }

    public long getStructBalanceNQT() {
        return getStructBalanceNQT(Qoober.getBlockchain().getHeight() - Constants.GUARANTEED_BALANCE_CONFIRMATIONS);
    }

    public long getStructBalanceNQT(int height) {
        Qoober.getBlockchain().readLock();
        try {
            if (height < 0)
                height = 0;
            if (height > Qoober.getBlockchain().getHeight())
                throw new IllegalArgumentException("Height " + height + " not available for struct balance calculation");

            try (Connection con = Db.db.getConnection();
                 PreparedStatement pstmt = con.prepareStatement("SELECT balance FROM account_struct WHERE id = ? " +
                     "AND height <= ? ORDER BY height DESC LIMIT 1"))
            {
                pstmt.setLong(1, this.id);
                pstmt.setInt(2, height);
                try (ResultSet rs = pstmt.executeQuery()) {
    ...
    public long getVirtualParaminingBalanceNQT() {
        Qoober.getBlockchain().readLock();
        try {
            return Paramining.calculate(this.id);
        } finally {
            Qoober.getBlockchain().readUnlock();
        }
    }
