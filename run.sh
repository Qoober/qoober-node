...
if [ $desktop -eq 1 ]; then
  ...
  DIR=`dirname "$0"`
  cd "${DIR}"
  nohup ${JAVACMD} -cp classes:lib/*:conf:addons/classes:addons/lib/*:javafx-sdk/lib/* qoober.Qoober > /dev/null 2>&1 &
  echo $! > ~/.nxt/nxt.pid
  cd - > /dev/null
else
  echo "Starting default mode"
  ${JAVACMD} -cp classes:lib/*:conf:addons/classes:addons/lib/*:javafx-sdk/lib/* qoober.Qoober
fi
