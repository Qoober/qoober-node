 ...
 *
 *   java -cp "classes:lib/*:conf" qoober.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dqoober.runtime.mode=desktop qoober.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Qoober properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "qoober.testDb" : "qoober.db";
        String dbType = Qoober.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Qoober.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Qoober.getDbDir(Qoober.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Qoober.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        String dbUsername = Qoober.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Qoober.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "qoober.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "qoober.mv.db");
            if (!dbFile.exists()) {
        ...
        try {
            ...
            switch (phase) {
                ...
                case 1:
                    ...
                    //
                    File newFile = new File(dbDir, "qoober.h2.db");
                    if (newFile.exists()) {
                        if (!newFile.delete()) {
                            Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                        }
                    } else {
                        newFile = new File(dbDir, "qoober.mv.db");
                        if (newFile.exists()) {
                            if (!newFile.delete()) {
                                Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                            }
