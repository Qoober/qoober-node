...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Qoober.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Qoober.getBlockchainProcessor().popOffTo(Qoober.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Qoober.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Qoober.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Qoober.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
