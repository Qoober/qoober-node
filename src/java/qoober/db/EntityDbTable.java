...
public abstract class EntityDbTable<T> extends TrimmableDbTable<T> {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Qoober.getBlockchainProcessor().isScanning() ?
                    Math.max(Qoober.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Qoober.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return Qoober.getBlockchain().getHeight() <= height && ! (isPersistent() && Qoober.getBlockchainProcessor().isScanning());
    }
