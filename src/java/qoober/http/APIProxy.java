...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Qoober.getBooleanProperty("qoober.enableAPIProxy") && ! API.isOpenAPI);
    private static final int blacklistingPeriod = Qoober.getIntProperty("qoober.apiProxyBlacklistingPeriod");
    static final String forcedServerURL = Qoober.getStringProperty("qoober.forceAPIProxyServerURL", "");
    public static final int PEER_CONNECTIONS_RETRIES = 3;
    private final List<String> bootstrapNodes = Qoober.getStringListProperty(
            Constants.isTestnet ? "qoober.testnetProxyBootstrapNodes" : "qoober.proxyBootstrapNodes");
    ...
    private JSONObject executeRequest(Peer peer, String requestType, String query) {
        ...
        try {
            uri.append("/qoober").append(requestType);
            if (query != null) {
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Qoober.getBlockchainProcessor().isDownloading());
    }
    ...
    boolean blacklistHost(String host) {
        ...
        int unblacklistTime = Qoober.getEpochTime() + blacklistingPeriod;
        blacklistedPeers.put(host, unblacklistTime);
