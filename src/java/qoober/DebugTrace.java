...
public final class DebugTrace {
    ...
    static final String QUOTE = Qoober.getStringProperty("qoober.debugTraceQuote", "\"");
    static final String SEPARATOR = Qoober.getStringProperty("qoober.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = Qoober.getBooleanProperty("qoober.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Qoober.getStringListProperty("qoober.debugTraceAccounts");
        String logName = Qoober.getStringProperty("qoober.debugTraceLog");
        ...
        Qoober.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Qoober.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Qoober.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Qoober.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            map.put("transaction fee", String.valueOf(-Constants.SHUFFLING_DEPOSIT_QNT));
            map.put("event", "shuffling blame");
            log(map);
            long fee = Constants.SHUFFLING_DEPOSIT_QNT / 4;
            ...
            fee = Constants.SHUFFLING_DEPOSIT_QNT - 3 * fee;
            long generatorId = Qoober.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(Qoober.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(Qoober.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Shuffling shuffling, boolean isRecipient) {
        ...
        String deposit = String.valueOf(isRecipient ? Constants.SHUFFLING_DEPOSIT_QNT : -Constants.SHUFFLING_DEPOSIT_QNT);
