...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        if (!Qoober.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("qooberdesktop.DesktopApplication").getMethod("launch").invoke(null);
        ...
        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("qooberdesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", Qoober.APPLICATION);
        addDataRow(statusPanel, "Version", Qoober.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(Qoober.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(Qoober.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", Qoober.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, Qoober.APPLICATION + " Server Status");
