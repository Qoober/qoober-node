...
import static qoober.http.JSONResponses.*;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Qoober.getBlockchain().getHeight() + 1,
                Qoober.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws QooberException {
        ...
        if (ecBlockId != 0 && ecBlockId != Qoober.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Qoober.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Qoober.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (QooberException.NotYetEncryptedException ignore) {}
        ...
        } catch (QooberException.NotYetEnabledException e) {
