...
public final class ShufflingCreate extends CreateTransaction {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) throws QooberException {
        ...
        if (holdingType == HoldingType.NXT && amount < Constants.SHUFFLING_DEPOSIT_QNT) {
            return JSONResponses.incorrect("amount", "Minimum shuffling amount is " + Constants.SHUFFLING_DEPOSIT_QNT / Constants.ONE_QBR + " " + Constants.COIN_SYMBOL);
        }
