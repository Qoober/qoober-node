...
import qoober.http.JSONResponses;
...
public abstract class AbstractEncryptedConfig implements AddOn {
    ...
    protected Path getDefaultPath() {
        Path path = Paths.get(Qoober.getStringProperty("qoober.addons.EncryptedConfig.path", "conf/processes/"), getDefaultFilename());
        if (!path.isAbsolute()) {
            path = Paths.get(Qoober.getUserHomeDir()).resolve(path).toAbsolutePath();
        }
        return path;
    }
