...
public final class Genesis {
    ...
    static void apply() {
        ...
        for (Map.Entry<String, Long> entry : ((Map<String, Long>)balances).entrySet()) {
            ...
            account.setGenesisParent();
            account.initParaminingTables();
        ...
        if (total > Constants.MAX_BALANCE_QNT) {
            throw new RuntimeException("Total balance " + total + " exceeds maximum allowed " + Constants.MAX_BALANCE_QNT);
        }
        Logger.logDebugMessage("Total balance %f %s", (double)total / Constants.ONE_QBR, Constants.COIN_SYMBOL);
