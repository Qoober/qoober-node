...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws QooberException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new QooberException.NotValidException("Invalid block signature");
            }
            return block;
        } catch (QooberException.NotValidException|RuntimeException e) {
    ...
    void apply() {
        ...
        if (this.height > 3) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f %s to forger at height %d", ((double)backFees[i])/Constants.ONE_QBR, Constants.COIN_SYMBOL, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f %s at height %d", ((double)totalBackFees)/Constants.ONE_QBR, Constants.COIN_SYMBOL, this.height);
        }
